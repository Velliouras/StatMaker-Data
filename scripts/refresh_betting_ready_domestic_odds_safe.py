...
REPORT = ROOT / "reports" / "betting_ready_domestic_odds_safe.json"
TARGET_CODES = {"CHN", "SRB", "CZE"}
...
def main() -> int:
    ...
    if not TARGET_CODES.issubset(by_code):
        missing = sorted(TARGET_CODES - set(by_code))
    ...
    fetch_config = {
        "version": live_payload.get("schemaVersion") or 31,
        "horizonDays": 21,
        "leagues": selected,
    ...
    merged["debug"] = {
        **(previous.get("debug") or {}),
        "safeBettingReadyRefreshAt": merged["generatedAt"],
        "safeBettingReadyTargets": sorted(TARGET_CODES),
        "safeBettingReadyRefreshed": refreshed,
        "safeBettingReadyPreserved": preserved,
        "safeBettingReadyEmptyFresh": empty_fresh,
        "safeBettingReadyRateLimitRemaining": debug.get("rateLimitRemaining"),
        "safeBettingReadyWarnings": debug.get("warnings", []),
    }
    ...
    save(OUT, merged)
    save(REPORT, {
        "mode": "betting-ready-targets-safe",
