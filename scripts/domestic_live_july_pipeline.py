#!/usr/bin/env python3
"""Maintain StatMaker Domestic leagues that are active or start during July.

The pipeline keeps statistics and betting availability independent:
- every selected league is published to the app-facing statistics index;
- betting data is emitted only when exact Odds-API.io odds and canonical team
  mapping exist;
- partial/rate-limited odds runs are merged into the previous usable artifact;
- the Android app continues to consume repository JSON only.
"""
...
def api_football_catalog(api_key: str) -> List[Dict[str, Any]]:
    request = Request(
        f"{API_FOOTBALL_BASE}/leagues?{urlencode({'current': 'true'})}",
        headers={
...
def select_target_season(
    ...
    candidates: List[Tuple[dt.date, Dict[str, Any], str]] = []
    for season in seasons:
        start, end = season_bounds(season)
        if start is None or end is None:
            continue
        if start <= today <= end:
            candidates.append((start, season, "active"))
        elif start.year == today.year and start.month == 7:
            candidates.append((start, season, "starts_in_july"))
    if not candidates:
        return None
...
def build_live_registry(
    ...
    for domestic in domestic_config.get("leagues", []) or []:
        ...
        try:
            ...
            continue
        season_choice = select_target_season(seasons_map.get(league_id, []), today)
        if season_choice is None:
            continue
        target, lifecycle = season_choice
        history = select_history_season(seasons_map.get(league_id, []), target, lifecycle)
        ...
        if history_year is None or target_year is None:
            continue

        selected.append({
            "leagueCode": str(domestic.get("leagueCode") or enrichment.get("leagueCode") or "").strip(),
            ...
            "football_data_code": enrichment.get("football_data_code") or domestic.get("leagueCode"),
            ...
            "app_season": app_season_label(target_start, target_end),
...
def cache_has_statistics(league: Dict[str, Any]) -> bool:
    ...
    return any(isinstance(item.get("normalized_stats"), dict) for item in fixtures or [] if isinstance(item, dict))
...
def fetch_stats_cycle(
    ...
    ordered = rotated(missing_first, cursor)
...
def build_app_statistics(registry: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for league in registry:
        artifact, row = enriched_build.build_league_artifact(league, min_fixtures=0, min_coverage=0.0)
    ...
    index.update({
        "selection_policy": "all configured leagues active now or starting during July",
...
def merge_odds_feed(
    ...
    merged["leagues"] = combined
    merged["debug"] = {
        **(previous.get("debug") or {}),
        **(fresh.get("debug") or {}),
        "mergePolicy": "replace successfully refreshed leagues; preserve unprocessed leagues; prune expired matches",
...
def fetch_odds_cycle(
    ...
    odds_leagues = [league for league in registry if bool(league.get("enabledForOdds", True))]
    cursor = int(state.get("oddsCursor") or 0)
    ordered = rotated(odds_leagues, cursor)
    selected = ordered[:max(1, cycle_size)]
    config = {
        "version": max(int(domestic_config.get("version") or 0), 4),
        "horizonDays": int(domestic_config.get("horizonDays") or 31),
        "leagues": selected,
    }
    ...
    if odds_leagues:
        state["oddsCursor"] = (cursor + len(selected)) % len(odds_leagues)
    state["lastOddsLeagues"] = [league.get("leagueCode") for league in selected]
...
def main() -> int:
    ...
    if not api_football_key:
        print("ERROR: API_FOOTBALL_KEY is required to discover active and July-starting leagues.", file=sys.stderr)
        return 2

    domestic_config = load_json(DOMESTIC_CONFIG, {})
    enrichment_config = load_json(ENRICHMENT_CONFIG, {})
    catalog = api_football_catalog(api_football_key)
    registry = build_live_registry(domestic_config, enrichment_config, catalog, today)
    if not registry:
        print("ERROR: live/July registry is empty; refusing to replace app-facing artifacts.", file=sys.stderr)
        return 3
    ...
    registry_payload = {
        ...
        "selectionPolicy": "configured leagues active now or whose target season starts during July",
        "statsVisibility": "all selected leagues are visible in app statistics even without odds",
        "bettingGate": "exact bookmaker odds and valid historical support are required",
    ...
    report = {
        ...
        "oddsLeaguesWithMatches": sum(1 for league in odds_feed.get("leagues", []) or [] if league.get("matches")),
        "state": state,
    }
